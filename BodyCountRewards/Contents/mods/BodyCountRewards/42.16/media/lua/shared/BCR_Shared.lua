...
local function getTraitUserdata(traitName)
    ...
    if success and result then
        traitUserdataCache[traitName] = result
        return result
    end
...
local function hasMutuallyExclusiveTrait(player, traitName)
    ...
    for _, exclTraitName in ipairs(exclusiveTraits) do
        local hasTrait = playerHasTrait(player, exclTraitName)
...
function BCR.filterPoolByExclusion(pool, excludeSet)
    ...
    for _, entry in ipairs(pool) do
        if not excludeSet[entry.trait] then
...
local function canEarnTrait(player, traitName)
    local hasTrait = playerHasTrait(player, traitName)
    if hasTrait ~= false then return false end
    if hasMutuallyExclusiveTrait(player, traitName) then return false end
    return true
end

local function canRemoveTrait(player, traitName)
    return playerHasTrait(player, traitName) == true
end
...
function BCR.addTraitToPlayer(player, traitEntry)
    ...
    if hasTrait ~= false then
        BCR.DebugPrint("addTraitToPlayer: Player already has '" .. traitName .. "' or error checking")
        return false
    end
    ...
    if not hasNow then
        BCR.DebugPrint("addTraitToPlayer: WARNING - " .. traitName .. " not confirmed after successful add call (engine may not have synced yet)")
    else
        BCR.DebugPrint("addTraitToPlayer: " .. traitName .. " added = true")
    end
...
function BCR.removeTraitFromPlayer(player, traitEntry)
    ...
    local hasTrait = playerHasTrait(player, traitName)
    if hasTrait ~= true then
        BCR.DebugPrint("removeTraitFromPlayer: Player doesn't have '" .. traitName .. "' or error checking")
        return false
    end
    ...
    local stillHas = playerHasTrait(player, traitName)
    if stillHas then
        BCR.DebugPrint("removeTraitFromPlayer: WARNING - " .. traitName .. " still present after successful remove call (engine may not have synced yet)")
    else
        BCR.DebugPrint("removeTraitFromPlayer: " .. traitName .. " removed = true")
    end
